...
;	main.c:35: ch = getchar();
	...
	mov	a,r6
	movx	@dptr,a
;	main.c:36: putchar(ch);
	mov	r7,#0x00
	mov	dpl,r6
	mov	dph,r7
	lcall	_putchar
;	main.c:37: if(check == 1)
	mov	dptr,#_check
	movx	a,@dptr
	mov	r7,a
	cjne	r7,#0x01,00102$
...
00190$:
	ljmp	00108$
00191$:
;	main.c:47: memset(x,'\0',10 * sizeof(char));
	mov	dptr,#_memset_PARM_2
	clr	a
	movx	@dptr,a
	mov	dptr,#_memset_PARM_3
...
;	main.c:48: printf_tiny("\n\rEnter the row number from 0 to 2\n\r");
	...
	dec	sp
	dec	sp
;	main.c:49: uint8_t row = getchar();
	lcall	_getchar
	mov	r6,dpl
;	main.c:50: putchar(row);
	mov	ar5,r6
	mov	r7,#0x00
	mov	dpl,r5
	mov	dph,r7
...
;	main.c:59: gets(x);
	...
	mov	b,#0x00
	lcall	_gets
;	main.c:60: uint8_t column = atoi(x);
	mov	dptr,#_main_x_196609_75
	mov	b,#0x00
	lcall	_atoi
	mov	r5,dpl
	pop	ar7
;	main.c:62: if(column > 15)
	mov	a,r5
	add	a,#0xff - 0x0f
	jnc	00106$
...
;	main.c:66: continue;
	ljmp	00128$
00106$:
;	main.c:68: uint8_t ad = lookUpTable1[row][column];
	mov	a,r7
	mov	b,#0x10
	mul	ab
	add	a,#_lookUpTable1
	...
	clr	a
	movc	a,@a+dptr
;	main.c:69: printf("%d %d %d", row, column, ad);
	mov	r6,a
	mov	r3,a
	mov	r4,#0x00
	mov	r2,#0x00
...
;	main.c:72: printf_tiny("\n\rEnter the character\n\r");
	...
	dec	sp
	dec	sp
;	main.c:73: uint8_t inputWrite = getchar();
	lcall	_getchar
	mov	r5,dpl
;	main.c:74: putchar(inputWrite);
	mov	ar4,r5
	mov	r7,#0x00
	mov	dpl,r4
	mov	dph,r7
	...
	pop	ar5
	pop	ar6
;	main.c:75: goToAddr(ad);
	mov	dpl,r6
	push	ar5
	lcall	_goToAddr
	pop	ar5
;	main.c:76: lcdPutCh(inputWrite);
	mov	dpl,r5
	lcall	_lcdPutCh
00108$:
;	main.c:79: if (ch == 'm')
...
;	main.c:115: printf_tiny("\n\rEnter the string\n\r");
	...
	dec	sp
	dec	sp
;	main.c:116: gets(stringRead);
	mov	dptr,#_main_stringRead_196609_87
	mov	b,#0x00
	lcall	_gets
;	main.c:118: putsLCD(stringRead);
	mov	dptr,#_main_stringRead_196609_87
	mov	b,#0x00
	lcall	_putsLCD
00122$:
...
;------------------------------------------------------------
;	main.c:134: void timer0_isr() __interrupt (1)
;	-----------------------------------------
;	 function timer0_isr
;	-----------------------------------------
_timer0_isr:
...
_putchar:
	...
	inc	dptr
	movx	@dptr,a
;	main.c:169: while ((SCON & 0x02) == 0)    // wait for TX ready, spin on TI
00103$:
	mov	a,_SCON
	jb	acc.1,00105$
;	main.c:171: if(check == 1)
	mov	dptr,#_check
	movx	a,@dptr
	mov	r7,a
	cjne	r7,#0x01,00103$
;	main.c:173: check = 0;
	...
	clr	a
	movx	@dptr,a
;	main.c:174: printTimeStamp();
	lcall	_printTimeStamp
	sjmp	00103$
00105$:
;	main.c:177: SBUF = c;  	// load serial port with transmit value
	mov	dptr,#_putchar_c_65536_96
	movx	a,@dptr
	mov	r6,a
	inc	dptr
...
;------------------------------------------------------------
;	main.c:182: int getchar ()
;	-----------------------------------------
;	 function getchar
;	-----------------------------------------
_getchar:
;	main.c:184: while ((SCON & 0x01) == 0)  // wait for character to be received, spin on RI
00103$:
	mov	a,_SCON
	jb	acc.0,00105$
;	main.c:186: if(check == 1)
	mov	dptr,#_check
	movx	a,@dptr
	mov	r7,a
	cjne	r7,#0x01,00103$
;	main.c:188: check = 0;
	...
	clr	a
	movx	@dptr,a
;	main.c:189: printTimeStamp();
	lcall	_printTimeStamp
	sjmp	00103$
00105$:
;	main.c:192: RI = 0;			// clear RI flag
;	assignBit
	clr	_RI
;	main.c:193: return SBUF;  	// return character from SBUF
	mov	r6,_SBUF
	mov	r7,#0x00
	mov	dpl,r6
	mov	dph,r7
...
;------------------------------------------------------------
;	main.c:196: void printTimeStamp()
;	-----------------------------------------
;	 function printTimeStamp
;	-----------------------------------------
_printTimeStamp:
;	main.c:198: goToAddr(0x57);
	mov	dpl,#0x57
	lcall	_goToAddr
;	main.c:199: lcdPutCh(hour + '0');
	mov	dptr,#_hour
	movx	a,@dptr
	add	a,#0x30
	mov	dpl,a
